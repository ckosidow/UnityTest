...
public class CharacterController2D : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
        ...
        int allColliders = colliders.Length;

        for (int i = 0; i < allColliders; i++)
		{
	...
	public void Move(float move, bool crouch, bool jump)
	{
        ...
        if (m_Grounded || (m_AirControl && !crouch))
		{
			...
			if (crouch)
			{
                ...
                if (m_Grounded)
                {
                    // Reduce the speed by the crouchSpeed multiplier if on the ground
                    move *= m_CrouchSpeed;
                }
        ...
        float sloapAngle = slope();

        if (sloapAngle != 0)
        {
            if (!m_Grounded)
            {
                if (sloapAngle < 0 && move < 0 || (sloapAngle > 0 && move > 0))
                {
                    m_Rigidbody2D.gravityScale = gravityScale * 10;
                }
            } else
            {
                if (move != 0)
                {
                    m_Rigidbody2D.gravityScale = gravityScale * .2f;
                }
            }
        } else
        {
